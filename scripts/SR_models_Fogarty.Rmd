...
dat.40.alpha$Location  <- factor(dat.40.alpha$Location, levels = c("North Sea","Irish Sea","Faroese","Barents Sea","Iceland","Eastern Scotian Shelf","Western Scotian Shelf","Eastern Georges Bank"))
...
The SSB and R time series were generally longer for the NE Atlantic stocks in comparison to the available data for the NW Atlantic stocks, attributable in part to earlier start time of the NE series (Table 1). The year SSB reached its minimum observed level did not show any particular pattern with the possible exception of three regions (Iceland, Barents Sea, and North Sea) where minima occurred prior to the 1990s (with the exception of North Sea cod). There was no pattern in the year of maximum SSB which ranged as early as 1955 for Icelandic cod to 2013 for cod and haddock in the Barents Sea and 2018 for Irish Sea haddock. Variability in SSB, based on the ratio of the maximum to minimum observed values in the time series, was lowest among the NE Atlantic haddock stocks (range: 4.4 – 10.7), followed by NE Atlantic cod stocks (range: 6.3 – 20.9), NW Atlantic haddock stocks (range: 3.1 – 25.2) and NW Atlantic cod stocks (range: 10 - 39).  Myers et al. (ref needed) have argued that poor fits of theoretical stock and recruitment relationships often arise due to the low dynamic range of observations so one might expect that relatively stronger S-R relationships will be evident among the NW Atlantic stocks. The Ricker models explained only `r r2.mean`% of the variance between recruits and SSB on average, with no significant difference observed between the Species, Regions, or the Period (Table 2). 

In nearly every region, the standard deviation of the log10 transformed recruitment series was higher for haddock than for cod (Table 1) suggesting the patterns were robust to the varying time series length and range of variation in SSB. The pattern is also broadly supportive of the finding by Fogarty et al. (2001) regarding higher recruitment variability among co-occurring stocks of haddock and cod, although no adjustments were made to account for the potential influence of spawning stock biomass differences among the two species with regions. 

R and SSB Time Trends

For cod across all regions, the recruit numbers in the *Recent* period were on average `r r.cod`% lower than the *Pre 1993* period, in the Northeast Atlantic the decline was `r r.cod.ne` while in the Northwest Atlantic this decline was more pronounced with declines in recruit numbers averaging `r r.cod.nw`% (Figures \@ref(fig:fog-rec) - \@ref(fig:hist-rec-ssb)). Similarly, the *SSB* for Atlantic Cod declined in the *Recent* period by `r ssb.cod`% across all regions, which was again driven by the Northwest Atlantic stocks where declines in *SSB* averaged `r ssb.cod.nw`% in the *Recent* period, the declines for the Northeast Atlantic stocks averaged `r ssb.cod.ne`% (Figures \@ref(fig:fog-rec) - \@ref(fig:hist-rec-ssb)). 

For haddock across all regions, the recruit numbers increased in the *Recent* period by `r r.had`% relative to the *Pre 1993* period, in the Northeast Atlantic there was no difference in the aveage recruitment numbers between the two perios, while in the Northwest Atlantic the recruit numbers averaged `r r.had.nw`% higher in the *Recent* period (Figures \@ref(fig:fog-rec) - \@ref(fig:hist-rec-ssb)).  Similarly, the *SSB* for Haddock increased in the *Recent* period by `r ssb.had`%, with the increase in *SSB* higher for the Northwest Atlantic stocks (`r ssb.had.nw`%) than the Northeast Atlantic stocks(`r ssb.had.ne`%).

## Recruitment residuals 

The results of the analysis using the standard deviation of the recruitment residuals from the Ricker Stock Recruitment models and the GAM models both indicated that recruitment variability was larger for Haddock than for Atlantic Cod (Table 2). The standard deviation of the recruitment residuals were on average `r ratio.pre`% higher (Ricker model residuals) for the Haddock Stocks in the *Pre 1993* period than the Atlantic Cod stocks (Figures \@ref(fig:sd-resid-sr), \@ref(fig:sd-resid-gam), and Table 2). In the *Recent* Period, the standard deviation of the recruitment residuals were on average `r ratio.recent`% higher (Ricker model residuals) for the Haddock Stocks than the Atlantic Cod stocks (Figures \@ref(fig:sd-resid-sr), \@ref(fig:sd-resid-gam),and Table 2), similar to the *Pre 1993* period. 

## Resilience: Steepness parameter

In the *Pre 1993* period, no generalizable differences were observed between the steepness parameter estimate (i.e. slope at origin, *log(a)* parameter in Ricker model) of Haddock and Atlantic Cod stocks using either the Ricker model estimates (Figure \@ref(fig:sr-alpha-plot) and Table 2) or when using the average log(R/SSB) when SSB is < 0.4 (Figure \@ref(fig:alpha-40-plt)). In the *Recent* period, the steepness parameter estimates of Haddock stocks tended to be higher than the Atlantic Cod stocks using both the Ricker model estimates (Figure \@ref(fig:sr-alpha-plot) and Table 2) and when using the average log(R/SSB) when SSB is < 0.4 (Figure \@ref(fig:alpha-40-plt)). In most cases the steepness estimates in the *Recent* period were not significantly different, but for all regions except the Eastern Scotian Shelf the steepness estimate was higher for Haddock than for Atlantic Cod. These results are in contrast to the results of Fogarty et al. (2001) in which the steepness parameter estimate tended to be higher for cod than observed in Haddock.

Residual Autocorrelation and Time series correlation

The autocorrelation in recruitment residuals in the *Pre 1993* tended to be positive for the Northeast Atlantic stocks (Figures \@ref(fig:acf-split-gam) and \@ref(fig:acf-split-sr)).  In the Northwest Atlantic, the more southern stocks tended to have a lower autocorrelation, with Eastern Georges Bank Atlantic Cod being the only stock in the analysis with negative residuals using both the GAM model and the Ricker model. The correlation between the Atlantic Cod and Haddock stocks tended to be positive in the *Pre 1993* period, but for most (4) stocks the correlation was not significant (Figure \@ref(fig:cross-ccf-split)). These results are broadly similar to the results found in Fogarty et al. (2001).

The autocorrelation in recruitment residuals in the *Recent* tended to be positive for the Northeast Atlantic stocks (Figures \@ref(fig:acf-split-gam) and \@ref(fig:acf-split-sr)), similar to what was observed in the *Pre 1993* period.  In the Northwest Atlantic, the Eastern Georges Bank and Western Scotian Shelf stocks tended to be more negative and had larger uncertainties than most other stocks for both the GAM and Ricker residuals. The correlation between the Atlantic Cod and Haddock stocks tended to be positive in the *Recent* period, but for most (6) stocks the correlation was not significant (Figure \@ref(fig:cross-ccf-split)).
...
```{r cross-ccf-split,echo=F,fig.width=10,fig.height=10,message=F,warning=F,fig.cap ="Correlation of the recruitment (log scale) time series between cod and haddock stocks in each region. "}
...
p.ccf.split
save_plot("D:/Github/Haddock/Results/Figure_3.png",p.ccf.split,base_height = 11, base_width = 8)
...
p.sds
save_plot("D:/Github/Haddock/Results/Figure_4.png",p.sds,base_height = 11,base_width = 8)
...
```{r acf-split-sr,echo=F,fig.width=10,fig.height=10,message=F,warning=F,fig.cap ="Autocorrelation of recruitment residuals from Ricker stock recruitment model in each Period. The dashed lines show the average for the North East Atlantic (left) and North West Atlantic (right) stocks. The error bars represent the 95//% Confidence Interval."}
...
acf.NW <- data.frame(Period = rep(acf.NW$Period,2), mn.cor = rep(acf.NW$mn.cor,2))

acf.NW$x <- c(11,11,16,16)
...
acf.NE <- data.frame(Period = rep(acf.NE$Period,2), mn.cor = rep(acf.NE$mn.cor,2))

acf.NE$x <- c(0,0,10,10)
...
p.acf.split.sr <- ggplot(dat.acf.split %>% dplyr::filter(Model == "Stock Recruit"),aes(y=Correlation,x=Stock,fill = Location,shape = Location,color=Location)) +
                facet_wrap(~Period,ncol=1) + geom_point(size=4) +                 
                geom_errorbar(aes(ymin =lci,ymax=uci,x = Stock,color=Location),width=0) + 
                scale_fill_manual(values = cols)+ scale_color_manual(values = cols)+ scale_shape_manual(values = shapes) +
...
save_plot("D:/Github/Haddock/Results/Figure_5.png",p.acf.split.sr,base_height = 11,base_width = 11)
...
```{r stan-alpha-plot,warning =F,echo=F,fig.width=11,fig.height=8,message=F,fig.cap = "The log of the maximum annual reproductive rate estimated using the Ricker stock recruitment model for each stock in the Pre-1993 and Recent period with 95\\% confidence intervals."}
...
d.merge$Location  <- factor(d.merge$Location, levels = c("North Sea","Faroese","Barents Sea","Iceland","Eastern Scotian Shelf","Western Scotian Shelf","Eastern Georges Bank"))


cols <- c("blue","green","grey","blue","firebrick2","green","grey")
shapes <- c(rep(21,3),rep(24,4))
...
alpha.stan.plt <- ggplot(d.merge,aes(y = alpha.had,x=alpha.cod,fill = Location,shape = Location,color=Location)) + 
     ...
     xlab("Cod standardized log(alpha)") + ylab("Haddock standardized log(alpha)") + 
     theme(legend.title = element_blank(),legend.position = 'top')
alpha.stan.plt

#write.csv(d.merge,file = "D:/Github/Haddock/Results/stan_alpha_values.csv")
save_plot(plot = alpha.stan.plt,filename = "D:/Github/Haddock/Results/Figure_6.png",base_height = 11,base_width = 8)
```
...
```{r sd-resid-gam,echo=F,warning=F,fig.width=10,fig.height=10,message=F,fig.cap = "Standard deviation of the log residuals from the Generalized Additive Models (GAMs)."}
...
save_plot("D:/Github/Haddock/Results/Figure_7.png",p.sds.gam,base_height = 11,base_width = 8)
...
```{r, alpha-40-plt,message =F, warning =F,echo=F,fig.width=10,fig.height=10,message=F,fig.cap = "The log of the maximum annual reproductive rate estimated using the mean recruitment when SSB is ≤ 0.4 of maximum SSB. The error bars represent 1 standard deviation from the mean.  Note that there was insufficient data in the recent period for Western Scotian Shelf Haddock to calculate the standard deviation."}
cols <- c("blue","firebrick2","green","grey","blue","firebrick2","green","grey")
shapes <- c(rep(21,4),rep(24,4))
alpha.40.plt <- ggplot(dat.40.alpha,aes(y = alpha.40.had,x=alpha.40.cod,fill = Location,shape = Location,color=Location)) + 
     ...
     theme(legend.title = element_blank(),legend.position = 'top')
alpha.40.plt
save_plot("D:/Github/Haddock/Results/Figure_9.png",alpha.40.plt,base_height = 11,base_width = 8)
...
acf.NW <- dat.acf.split %>% 
                       dplyr::filter(Location %in% c("Eastern Georges Bank","Western Scotian Shelf","Eastern Scotian Shelf"),Model != "GAM") %>% 
                       dplyr::group_by(Period) %>%
                       dplyr::summarise(mn.cor = mean(Correlation,na.rm=T))
acf.NW <- data.frame(Period = rep(acf.NW$Period,2), mn.cor = rep(acf.NW$mn.cor,2))
...
p.acf.split.gam <- ggplot(dat.acf.split %>% dplyr::filter(Model == "GAM"),aes(y=Correlation,x=Stock,fill = Location,shape = Location,color=Location)) +
                facet_wrap(~Period,ncol=1) + geom_point(size=4) + scale_fill_manual(values = cols)+ scale_color_manual(values = cols)+scale_shape_manual(values = shapes) +                 geom_errorbar(aes(ymin =lci,ymax=uci),width=0) + 
                geom_line(data = acf.NE, aes(y = mn.cor,x=x),color='purple',linetype='dashed',size=1,alpha = 0.5) + 
                geom_line(data = acf.NW, aes(y = mn.cor,x=x),color='darkgrey',linetype='dashed',size=1,alpha=0.5) + 
                scale_y_continuous(limits=c(-1,1))+
                theme(axis.text.x = element_text(angle = 45, hjust = 1.,size=16), legend.title = element_blank(),legend.position = 'top') + xlab("")
...
save_plot("D:/Github/Haddock/Results/Figure_A1.png",p.acf.split.gam,base_height = 11,base_width = 11)
